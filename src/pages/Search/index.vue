...
/*     isAsc(){
  ...
  methods: {
    //向服务器发请求search模块数据（根据参数不同展示不同的数据）
    getData() {
    ...
    //排序的操作
    changeOrder(flag){
      //flag形参：代表用户点击的是综合还是价格 由用户点击的时候传进来的
      // let originOrder = this.searchParams.order;  //获取到初始状态，综合还是价格？升序还是降序 这里默认为 1，desc 综合降序
      let originFlag = this.searchParams.order.split(":")[0]; //这里判断是综合还是价格  即order第一个参数为1还是2
      let orginSort = this.searchParams.order.split(":")[1];  //这里判断是升序还是降序
      //准备一个新的order属性值
      let newOrder = "";
      //确定点击的一定是综合
      if(flag == originFlag){
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      //再次发起ajax请求
      this.getData()
      //每一次请求完毕，应该把1，2，3级分类的id清空，让他可以接收下一次的1，2，3级id
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
    },
